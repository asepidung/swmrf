...
* **Notifikasi Tugas Antar Modul:** Pengingat adanya tugas baru (misal PO Sapi menjadi tugas untuk Penerimaan Sapi) menggunakan komponen **Livewire Global Poller** yang ditempatkan pada *render hook* aplikasi (tanpa menggunakan tabel notifikasi khusus di *database*). Komponen ini secara mandiri melakukan *polling* (setiap 5 detik) untuk mengecek jika ada data terbaru (misalnya dengan mengecek `MAX(id)`) dan kemudian memicu *toast notification* melayang secara dinamis. Selain itu, sediakan juga sebuah Widget *banner* peringatan statis pada Dashboard menggunakan Eloquent *query* biasa (seperti `doesntHave()`) tanpa *polling* untuk menghitung sisa tugas yang belum dikerjakan.
* **Partial Processing (Proses Mencicil):** Untuk modul yang memungkinkan pengerjaan dicicil (misal: 1 data Timbang Sapi memiliki 50 ekor, namun dipotong (Carcass) menjadi 2 gelombang), halaman pembuatan data (Create) wajib secara dinamis menyiapkan *row* hanya untuk *item* yang belum diproses. *Item* yang sudah diproses di transaksi sebelumnya tidak boleh muncul lagi. Gunakan *sub-query* atau relasi `doesntHave` terhadap tabel anak transaksi lanjutan untuk mendapatkan sisa *item* tersebut. Halaman *Index* dari modul lanjutan juga memerlukan view khusus (seperti halaman Draft) yang memuat daftar dokumen induk yang belum terselesaikan 100%.
