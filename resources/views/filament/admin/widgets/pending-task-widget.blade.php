<x-filament-widgets::widget class="fi-wi-pending-task">
    @if($this->getPendingReceivingCount() > 0 || $this->getPendingWeighingCount() > 0 || $this->getPendingCarcassCount() > 0 || $this->getPendingMaterialRequestCount() > 0 || $this->getPendingMaterialFinanceCount() > 0)
    <div class="space-y-2">
        ...
        <div class="p-3 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center gap-x-3">
            ...
            <p class="text-sm font-medium text-gray-700 dark:text-gray-200">
                {!! __('Ada :count request material yang belum di-review oleh Purchasing.', ['count' => '<strong style="color: #f59e0b !important;">'.$this->getPendingMaterialRequestCount().'</strong>']) !!}
            </p>
        </div>
        @endif
