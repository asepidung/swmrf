...
<head>
    <meta charset="utf-8">
    <title>Aging Beef Stock (> 60 Days)</title>
    <style>
...
<body>
    <div class="header">
        <h2>Aging Beef Stock (> 60 Days)</h2>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $record->pack_date ? \Carbon\Carbon::parse($record->pack_date)->format('d-M-y') : '' }}</td>
                <td class="text-center">{{ $record->pack_date ? abs((int) now()->diffInDays($record->pack_date)) : '' }}</td>
            </tr>
            @empty
            <tr>
