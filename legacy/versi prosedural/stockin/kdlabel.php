...
$origin = 7; // Pastikan nilai origin tersedia
...
$sql = mysqli_query($conn, "SELECT COUNT(*) AS total FROM stockin WHERE YEAR(creatime) = YEAR(CURRENT_DATE)");
...
// Format nomor urut menjadi 9 digit
$seriallabel = sprintf("%012s", $kode);

// Menggabungkan prefix dengan nomor urut untuk menghasilkan ID lengkap
$kodeauto = $origin . $prefix . $seriallabel;
